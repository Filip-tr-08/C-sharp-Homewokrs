...
namespace CSharpAdvanceHomeworkFive.Services.Implementations
{
    ...
    {
        private FileDatabase<T> _users;
        public UserServices()
        {
            _users = new FileDatabase<T>();
        }
       ...
       public void AddUser(T user)
        ...
        public void UpdateListOfActivities(object activity, int userId)
        {
            T user = _users.GetById(userId);
            if (activity is Other hobby)
            {
                user.listOfOtherActivities.Add(hobby);
            }
            else if(activity is ReadingActivity reading)
            {
                user.listOfReadingActivities.Add(reading);

            }
            else if (activity is ExercisingActivity exercise)
            {
                user.listOfExerciseActivities.Add(exercise);

            }
            else if (activity is WorkingActivity working)
            {
                user.listOfWorkingActivities.Add(working);
